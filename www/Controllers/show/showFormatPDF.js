...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        const response = await fetch(allQuotes_route); // Cambia la URL según la configuración de tu API.
        const quotes = await response.json();

        // Filtrar la cotización por clientId y quoteId
        ...
        if (filteredQuote) {
            ...
            const row = `
                <tr style="height:41.5pt">
                    ...
                    <td style="width:43.35pt; padding: 5.4pt; vertical-align:top; background-color:#f2f2f2; text-align:center;">
                        <p style="margin-bottom:0pt; font-size:8pt">1</p>
                    </td>
            ...
            // Calcular el IVA y los totales
            const ivaPercentage = filteredQuote.PercentageIVA;
            const ivaAmount = (filteredQuote.Price * ivaPercentage) / 100;
            const totalInclIVA = filteredQuote.Price + ivaAmount;

            document.getElementById("totalPrice").innerText = `$${filteredQuote.Price.toFixed(2)}`;
            document.getElementById("subtotal").innerText = `$${filteredQuote.Price.toFixed(2)}`;
